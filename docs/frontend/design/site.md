...
Build new pages from the established menubar/contact language: shared near-black navy background, constrained content aligned to the menubar width, dark glass panels, restrained blue/violet/purple accents, and no page-wide mockup images. Put reusable visual primitives in `frontend/src/styles/_site-primitives.scss` under `site-*` classes, and keep route styles focused on layout and sizing. Use `sample_&_raw_assets/` images as references only; keep raw assets ignored and keep production app assets limited to brandmarks unless a future art pass explicitly promotes page artwork.
